...
const add = (ops, callback) => {
    ...
    db.query('select count(id) as c from t_users where name = ?', [user.name], (error, results, fields) => {
        ...
        db.query('insert into t_users values(?,?,?,?,FROM_UNIXTIME(?),FROM_UNIXTIME(?))', [user.id, user.name, enpwd.ecodedpwd, enpwd.salt, now.getUnixTime(), null], callback);
    });
...
const update = (ops, callback) => {
    let user = ops.user;
    db.query('select count(id) as c from t_users where name = ? and id != ?', [user.name, user.id], (error, results, fields) => {
        log.debug(JSON.stringify(results));
        if (results[0].c > 0) {
            callback(new Error('name already exists.'));
            return;
        ...
        var now = new Date();
        db.query('update t_users set name = ?, password = ?, salt = ?, update_time = ?  where id = ?',
            [user.name, user.password, user.salt, now.getUnixTime(), user.userId], callback);
    });
